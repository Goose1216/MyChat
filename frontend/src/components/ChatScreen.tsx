    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 p-4 max-w-4xl mx-auto w-full overflow-y-auto">
        <div className="bg-white rounded shadow p-4 space-y-3">
          {messages.length === 0 && <div className="text-gray-500 text-center py-10">Нет сообщений</div>}
          {messages.map((m) => (
            <div key={m.id} className={`flex ${m.is_self ? "justify-end" : "justify-start"}`}>
              <div className={`p-3 rounded-2xl max-w-[70%] ${m.is_self ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-900"}`}>
                <div>{m.text}</div>
                <div className="text-xs mt-1 opacity-70">{new Date(m.timestamp).toLocaleTimeString()}</div>
              </div>
            </div>
          ))}
          <div ref={bottomRef} />
