...
function validateFile(file: File): string | null {
  ...
  if (BLOCKED_EXTENSIONS.has(ext)) return `Файлы с расширением «${ext}» запрещены к отправке.`;
...
export default function ChatScreen({ userId, chat, onBack }) {
  ...
  // Нормализуем объект чата ПЕРВЫМ
  const safeChat = {
    ...chat,
  ...
  // Роль текущего пользователя (null = ещё не загружено — не показываем баннер)
  ...
  // Модалка управления ролями (только для канала + owner)
  ...
  // ── Производные флаги канала ─────────────────────────────────────────────
  const isChannel    = safeChat.chat_type === "channel";
  // null = загружается → оптимистично разрешаем писать пока не знаем
  const canWrite     = !isChannel || myRole === null || myRole === "owner" || myRole === "admin";
  const canManage    = isChannel && (myRole === "owner" || myRole === "admin");
  const isOwner      = myRole === "owner";
  const canManageTasks = !isChannel || myRole === "owner" || myRole === "admin";
  ...
  // ── Загрузка сообщений, участников, роли ────────────────────────────────
  useEffect(() => {
    (async () => {
    ...
    const handler = (msg: any) => {
      ...
      if (msg.type_of_message === 0) {
        ...
        if (msg.sender_id !== userId) {
          setIsAtBottom(prev => { if (!prev) setUnreadCount(c => c + 1); return prev; });
        }
      }
  ...
  // ── Начальный скролл к последнему прочитанному ───────────────────────────
  useEffect(() => {
    if (initialScrollDoneRef.current || !messages.length) return;
    if (safeChat.last_read_message_id) {
      const el = messageRefs.current[safeChat.last_read_message_id];
  ...
  // ── last_read_message_id sync ────────────────────────────────────────────
  useEffect(() => { lastReadMessageIdRef.current = safeChat.last_read_message_id || 0; }, [chat.id]);
  useEffect(() => { batchFromIdRef.current = safeChat.last_read_message_id || 0; }, [chat.id]);

  // ── Scroll tracker (read + isAtBottom) ──────────────────────────────────
  useEffect(() => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (currentReadId > batchFromIdRef.current) sendReadBatchRef.current(currentReadId);
    }, BATCH_INTERVAL_MS);
    return () => {
  ...
  const startEdit = (m: any) => { if (!m.is_self || m.file) return; setEditingId(m.id); setEditingText(m.text); };
  ...
  const sendFile = async () => {
    ...
    if (clientError) { setFileError(clientError); return; }
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        setFileError(data?.detail || data?.description || data?.errors?.[0]?.message || "Не удалось отправить файл");
        return;
      }
      setSelectedFile(null); setFileError(null);
    } catch { setFileError("Ошибка соединения. Попробуйте ещё раз."); }
  };
  ...
  const createTask = async () => {
    if (isChannel && !canManageTasks) { alert("Недостаточно прав"); return; }
  ...
  useEffect(() => {
    if (!readersModal) { if (readersRefreshRef.current) clearInterval(readersRefreshRef.current); return; }
    ...
    return () => { if (readersRefreshRef.current) clearInterval(readersRefreshRef.current); };
  }, [readersModal?.messageId]);

  // ── Role management ──────────────────────────────────────────────────────
  const loadMembersWithRoles = async () => {
    setLoadingRoles(true); setRoleChangeError("");
    try {
      const res  = await fetchWithAuth(`${API}/chats/${chat.id}/members_with_roles/`);
      const data = await res.json();
      setMembersWithRoles(data.data ?? []);
    } catch { setRoleChangeError("Ошибка загрузки участников"); }
  ...
  const changeRole = async (memberId: number, newRole: string) => {
    setRoleChangeError("");
    try {
      const res = await fetchWithAuth(`${API}/chats/${chat.id}/members/${memberId}/role/`, {
        method: "PATCH", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ role: newRole }),
      });
      if (!res.ok) {
        const d = await res.json().catch(() => ({}));
        setRoleChangeError(d?.detail || d?.description || "Ошибка смены роли");
        return;
      }
      setMembersWithRoles(prev => prev.map(m => m.id === memberId ? { ...m, role: newRole } : m));
    } catch { setRoleChangeError("Ошибка соединения"); }
  };
  ...
  const renderMessageContent = (m: Message) => {
      ...
      if (isImageFile(m.file)) {
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <img
              src={m.file.url} alt={m.file.filename} loading="lazy"
              onClick={() => setPreviewImage(m.file.url)}
              style={{ maxWidth: 320, maxHeight: 260, borderRadius: 12, cursor: "pointer", objectFit: "cover", border: "1px solid rgba(255,255,255,0.08)" }}
            />
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
              <span style={{ fontSize: 11, opacity: 0.7, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                🖼 {m.file.filename}
              </span>
              <a href={m.file.url} download={m.file.filename} target="_blank" rel="noopener noreferrer"
                style={{ fontSize: 11, fontWeight: 600, color: m.is_self ? "rgba(255,255,255,0.9)" : "var(--c-brand)", textDecoration: "none", flexShrink: 0 }}>
                Скачать
              </a>
            </div>
          </div>
    ...
    if (editingId === m.id) {
      return (
        <input
          style={{ background: "rgba(255,255,255,0.2)", border: "1px solid rgba(255,255,255,0.4)", borderRadius: 6, padding: "3px 8px", color: "#fff", fontSize: 13, outline: "none", minWidth: 160 }}
          value={editingText} autoFocus
          onChange={e => setEditingText(e.target.value)}
          onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); saveEdit(); } if (e.key === "Escape") { setEditingId(null); setEditingText(""); } }}
    ...
    <div style={s.page}>
      ...
      {previewImage && (
        <div onClick={() => setPreviewImage(null)}
          style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.88)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, cursor: "zoom-out", padding: 24 }}>
          <img src={previewImage} alt="preview" onClick={e => e.stopPropagation()}
            style={{ maxWidth: "95vw", maxHeight: "95vh", borderRadius: 16, objectFit: "contain", boxShadow: "0 20px 60px rgba(0,0,0,0.5)" }} />
        </div>
      )}
      ...
      <header style={s.header}>
        ...
        <div style={s.headerRight}>
          ...
          {/* Кнопка управления ролями — только владелец канала */}
          {isOwner && isChannel && (
            <button className="btn" style={{ fontSize: 12, background: "var(--c-brand-bg)", borderColor: "var(--c-brand-border)", color: "var(--c-brand)" }}
              onClick={() => { setManageRolesOpen(true); loadMembersWithRoles(); }}>
              👑 Роли
            </button>
          )}
            ...
            <div key={m.id} ref={el => (messageRefs.current[m.id] = el)}
              ...
              {!m.is_self && (
                <button onClick={() => setProfileUser(m.sender)}
                  ...
                  style={{ cursor: "pointer", flexShrink: 0, marginBottom: 2 }} title={safeName(m.sender)}>
                  {safeName(m.sender)[0]}
                </button>
              )}
      ...
      {!isAtBottom && (
        <div style={s.scrollBtnWrap}>
          <button onClick={scrollToBottom} style={s.scrollBtn} title="Вниз">
            ...
            {unreadCount > 0 && <span style={s.scrollBtnBadge}>{unreadCount > 99 ? "99+" : unreadCount}</span>}
          </button>
        </div>
      )}

      {/* ── Channel read-only banner ── */}
      {isChannel && !canWrite && myRole !== null && (
      ...
      {/* ── FOOTER ── */}
      <footer style={{ ...s.footer, display: isChannel && !canWrite && myRole !== null ? "none" : undefined }}>
        {typingIds.length > 0 && (
          <div style={s.typing}>
        ...
        {fileError && (
          <div style={s.fileErrorBanner}>
            ...
            <button style={s.fileErrorClose} onClick={() => { setFileError(null); setSelectedFile(null); }} title="Закрыть">✕</button>
          </div>
        )}
        ...
        <form onSubmit={e => { e.preventDefault(); selectedFile ? sendFile() : send(e); }} style={s.inputRow}>
          <input type="file" id="file-input" style={{ display: "none" }}
            ...
            onChange={e => {
              ...
              if (err) { setFileError(err); setSelectedFile(null); } else { setFileError(null); setSelectedFile(f); }
            }}
          />
          <div style={{ position: "relative" }} className="attach-wrap">
            <label htmlFor="file-input" style={{ ...s.attachBtn, background: selectedFile ? "var(--c-brand-bg)" : "var(--c-surface)", borderColor: selectedFile ? "var(--c-brand-border)" : "var(--c-line)" }}>📎</label>
            <div style={s.attachTooltip}>
              ...
              <div style={{ color: "var(--c-warning)", fontWeight: 700 }}>⚠ Максимальный размер: {MAX_FILE_MB} МБ</div>
            </div>
          </div>
          <input value={newMessage} onChange={e => { setNewMessage(e.target.value); sendTyping(); }}
        ...
        <div className="modal-backdrop">
          <div className="modal" style={{ maxWidth: 460 }}>
            <div className="modal-header">
              <span className="modal-title">👑 Управление ролями участников</span>
              <button className="modal-close" onClick={() => { setManageRolesOpen(false); setRoleChangeError(""); }}>✕</button>
            </div>
            {loadingRoles ? (
              <div style={{ display: "flex", justifyContent: "center", padding: "24px 0" }}><span className="spinner" /></div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                {membersWithRoles.map((m, i) => {
                  const name     = m.username || m.email || `#${m.id}`;
                  const isMe     = m.id === userId;
                  const mIsOwner = m.role === "owner";
                  const roleLabel: Record<string, string> = { owner: "👑 Владелец", admin: "✏️ Редактор", member: "👁 Читатель" };
                  return (
                    <div key={m.id} style={{
                      display: "flex", alignItems: "center", gap: 10, padding: "10px 4px",
                      borderBottom: i < membersWithRoles.length - 1 ? "1px solid var(--c-line-soft)" : "none",
                    }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 600, color: isMe ? "var(--c-brand)" : "var(--c-ink)" }}>
                          {name}{isMe && <span style={{ fontSize: 10, color: "var(--c-ink-muted)", fontWeight: 400, marginLeft: 6 }}>это вы</span>}
                        </div>
                        <div style={{ fontSize: 11, color: "var(--c-ink-muted)", marginTop: 1 }}>{roleLabel[m.role] ?? m.role}</div>
                      </div>
                      {isOwner && !mIsOwner && !isMe && (
                        <select value={m.role} className="input"
                          style={{ width: "auto", fontSize: 12, padding: "4px 24px 4px 8px", height: 30 }}
            ...
            {roleChangeError && <div className="alert alert-error" style={{ marginTop: 12 }}>{roleChangeError}</div>}
            <div style={{ marginTop: 16, padding: "10px 12px", background: "var(--c-surface)", borderRadius: "var(--r-md)", fontSize: 11, color: "var(--c-ink-muted)", lineHeight: 1.6 }}>
              <strong>👑 Владелец</strong> — создатель канала, управляет ролями<br/>
              <strong>✏️ Редактор</strong> — может писать сообщения и создавать задачи<br/>
              <strong>👁 Читатель</strong> — только просматривает
            </div>
          </div>
        </div>
      )}
      ...
      {tasksModalOpen && (
        <div className="modal-backdrop">
          <div className="modal" style={{ maxWidth: showStats ? 700 : 520, maxHeight: "90vh", overflow: "hidden", display: "flex", flexDirection: "column", transition: "max-width 0.25s ease" }}>
              ...
              {!showStats && (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  {!loadingTasks && tasks.map(t => {
                    ...
                    return (
                      <div key={t.id} style={{ border: `1px solid ${isMeAssigned ? "var(--c-green-border)" : "var(--c-line)"}`, borderRadius: "var(--r-md)", padding: 12, background: isMeAssigned ? "var(--c-green-bg)" : "var(--c-paper)" }}>
                        {editingTaskId === t.id ? (
                          ...
                          <>
                            ...
                            <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" as const }}>
                              ...
                              ) : <span className="pill">{PRIORITY_LABELS[t.priority] ?? t.priority}</span>}
                              {isCreator(t) && (
      ...
      {ctxMenu && (
        <>
          <div style={{ position: "fixed", inset: 0, zIndex: 99 }} onClick={closeCtxMenu} onContextMenu={e => { e.preventDefault(); closeCtxMenu(); }} />
          <div style={{
            position: "fixed",
            ...
            zIndex: 100, minWidth: 180, overflow: "hidden", animation: "modalIn 120ms ease",
          }} onClick={closeCtxMenu}>
            <div style={{ padding: "4px 0" }}>
              <button style={ctxStyle.item} onClick={() => { loadReaders(ctxMenu.messageId, ctxMenu.senderId); closeCtxMenu(); }}>
                <span>👁</span> Кто прочитал
              </button>
              {ctxMenu.isSelf && (
                <button style={{ ...ctxStyle.item, color: "var(--c-danger)" }} onClick={() => { deleteMessage(ctxMenu.messageId); closeCtxMenu(); }}>
                  <span>🗑</span> Удалить
                </button>
              )}
              {!ctxMenu.isDeleted && canManageTasks && (
                <button style={ctxStyle.item} onClick={() => { setTaskModalOpen(true); setTaskMessageId(ctxMenu.messageId); closeCtxMenu(); }}>
          ...
          <div className="modal" style={{ maxWidth: 360 }}>
            ...
            {readersModal.loading ? (
              ...
              <div style={{ textAlign: "center", padding: "24px 0", color: "var(--c-ink-muted)", fontSize: 13 }}>
                <div style={{ fontSize: 28, marginBottom: 8 }}>👁</div>Никто ещё не прочитал
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                {readersModal.readers.map((r, i) => {
                  ...
                  const readTime = r.read_at ? new Date(r.read_at).toLocaleString("ru-RU", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }) : "—";
                  return (
                    <div key={r.user_id} style={{
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 600, color: isMe ? "var(--c-brand)" : "var(--c-ink)" }}>
                          {name}{isMe && <span style={{ fontSize: 10, fontWeight: 400, color: "var(--c-ink-muted)", marginLeft: 6 }}>это вы</span>}
                        </div>
                        <div style={{ fontSize: 11, color: "var(--c-ink-muted)", marginTop: 1 }}>прочитано в {readTime}</div>
                      </div>
      ...
      <style>{`
        ...
        .msg-actions { display: none; position: absolute; top: -20px; right: 0; gap: 4px; background: var(--c-paper); border: 1px solid var(--c-line); border-radius: var(--r-md); padding: 3px 5px; box-shadow: var(--shadow-sm); }
        .msg-wrap:hover .msg-actions { display: flex; }
        .msg-action-btn { border: none; border-radius: var(--r-sm); width: 24px; height: 24px; font-size: 11px; display: flex; align-items: center; justify-content: center; cursor: pointer; background: transparent; }
...
const s: Record<string, React.CSSProperties> = {
  ...
  header:     { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "10px 16px", background: "var(--c-paper)", borderBottom: "1px solid var(--c-line)", flexShrink: 0, flexWrap: "wrap" as const },
  headerLeft: { display: "flex", alignItems: "center", gap: 10 },
  chatAvatar: { width: 36, height: 36, borderRadius: "50%", background: "var(--c-brand)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, fontFamily: "var(--font-mono)", flexShrink: 0 },
  ...
  messages:   { flex: 1, overflowY: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 10, background: "var(--c-surface)", backgroundImage: "radial-gradient(circle at 100% 0%, rgba(123,31,162,0.03) 0%, transparent 50%)", position: "relative" as const },
  ...
  attachBtn:  { width: 40, height: 40, border: "1.5px solid var(--c-line)", borderRadius: "var(--r-full)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, cursor: "pointer", background: "var(--c-surface)", flexShrink: 0, transition: "background var(--t-fast)" },
  channelReadOnly: { display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "10px 16px", background: "var(--c-surface)", borderTop: "1px solid var(--c-line)", fontSize: 12, color: "var(--c-ink-muted)", fontStyle: "italic" },
  scrollBtnWrap: { position: "absolute" as const, bottom: 90, right: 24, zIndex: 20, display: "flex", flexDirection: "column" as const, alignItems: "center", gap: 4, pointerEvents: "none" as const },
  scrollBtn:  { width: 40, height: 40, borderRadius: "var(--r-full)", background: "var(--c-paper)", border: "1.5px solid var(--c-line)", boxShadow: "var(--shadow-md)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "var(--c-brand)", pointerEvents: "auto" as const, transition: "background var(--t-fast), transform var(--t-fast)", position: "relative" as const },
  scrollBtnBadge: { position: "absolute" as const, top: -6, right: -6, background: "var(--c-brand)", color: "#fff", borderRadius: "var(--r-full)", fontSize: 9, fontWeight: 700, fontFamily: "var(--font-mono)", padding: "1px 5px", minWidth: 16, textAlign: "center" as const, border: "1.5px solid var(--c-paper)" },
  fileErrorBanner: { display: "flex", alignItems: "center", gap: 8, background: "var(--c-danger-bg)", border: "1px solid #FFCDD2", borderRadius: "var(--r-md)", padding: "8px 12px", marginBottom: 8, fontSize: 12, fontWeight: 600, color: "var(--c-danger)", animation: "fadeInDown 0.2s ease" },
  fileErrorClose: { background: "transparent", border: "none", cursor: "pointer", color: "var(--c-danger)", fontSize: 13, padding: "0 2px", flexShrink: 0 },
  filePreview: { display: "flex", alignItems: "center", gap: 10, background: "var(--c-brand-bg)", border: "1px solid var(--c-brand-border)", borderRadius: "var(--r-md)", padding: "7px 12px", marginBottom: 8 },
  ...
  attachTooltip: { display: "none", position: "absolute", bottom: "calc(100% + 8px)", left: 0, width: 260, background: "var(--c-ink)", color: "#fff", borderRadius: "var(--r-md)", padding: "10px 12px", fontSize: 11, boxShadow: "var(--shadow-md)", zIndex: 20, pointerEvents: "none", lineHeight: 1.5 },
};

const ctxStyle: Record<string, React.CSSProperties> = {
  item: { display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "9px 16px", background: "transparent", border: "none", fontSize: 13, fontWeight: 600, color: "var(--c-ink)", cursor: "pointer", textAlign: "left" as const, transition: "background var(--t-fast)" },
};